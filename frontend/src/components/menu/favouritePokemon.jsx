...
function FavouritePokemon() {
  ...
  const [favouritePokemons, setPokemons] = useState(null);
  ...
  async function getData(id_user) {
    try {
      const response = await axiosUserPokemon.post("getFavouritePokemon", {
        id_user: id_user,
      });
  ...
  if (!favouritePokemons || favouritePokemons.length === 0) {
    
    return <>
    <h1>{t("fav_title")}</h1>
    <h3 className="no_pokemon">{t("no_fav_pokemon")}</h3>
    </>
  }
